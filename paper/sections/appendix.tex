...
\paragraph{精确收缩基线。}
本文的精确收缩基线 exact-TNC 与 \method{} 共用同一张量网络对象、同一开放标签顺序以及同一组输出块。
对每个输出块 $\Omega_b$，exact-TNC 直接调用全网络精确收缩
\[
X_{\Omega_b} = \mathrm{contract\_full}(\mathcal N,\Omega_b),
\]
再将结果写回完整输出张量的对应位置。
其底层实现基于 \texttt{opt\_einsum} 的精确张量收缩接口，并按给定的 \texttt{optimize} 路径策略执行。
因此，\method{} 与 exact-TNC 的主要差异不在输出接口，而在于 \method{} 以划分树上的分隔状态递归替代了内部全量收缩过程。
...
\end{aligned}
\]
正文中的默认 \method{} 即中等强度预设 \textsf{L2}。
...
对每个网络实例，先生成同一组输出块，然后在该分块上分别运行 exact-TNC 与近似方法；因此不同方法面对的是相同的输出组织与写回负担。
...
在补强实验中，正文所引用的 Grid $3\times 3$ 默认配置对应同一批 core runs：方法为中等强度预设 \textsf{L2}，沿前两个输出轴按单索引块逐块生成完整输出，同时启用隐式草图合并与块间缓存复用。
