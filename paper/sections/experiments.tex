...
为了把缓存复用的收益更直观地呈现出来，我们进一步给出一张成对对照图。由 \Cref{fig:cache_reuse_pairs_grid} 可见，在对齐设置下，cache 在 $b2\_c1$（正文默认配置）与 $b2\_c2$ 两组设置上都带来稳定的降时效果；其中默认配置 $b2\_c1$ 的提升最明显。这种 paired comparison 比单独报一个命中率数字更直观，也和上面的机制表形成了互补。更完整的 blockwise 设计空间与机制指纹可视化见附录~\Cref{fig:mechanism_fingerprint_grid,fig:blockwise_operating_points_grid}。

\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{figures_strengthened/fig_cache_reuse_pairs_grid.pdf}
\caption{Grid $3\times 3$ 上 cache reuse 的成对对照。对齐设置下，开启 cache 在 $b2\_c1$（正文默认配置）和 $b2\_c2$ 上都带来稳定降时。}
