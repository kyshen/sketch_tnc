...
在分割树的不同位置，节点对最终输出的影响并不相同，因此对所有节点使用统一的局部容忍度并不理想。我们的基本想法是：对树中更深的节点，可以允许相对更宽松的局部近似；而对开放标签更多、与输出侧耦合更强的节点，则应采用更严格的误差控制。基于这一考虑，我们对节点级近似预算进行结构自适应分配。
具体地，对分割树中的任一节点 $v$，记其深度为 $d(v)$，开放标签数为 $o(v)$，则其局部容忍度定义为
\[
\tau_v=\tau \cdot \gamma^{d(v)}\big/\max(1,o(v))^{\beta},
\]
在实现中，叶节点与内部合并节点可分别使用不同的容忍度，记为 $\tau_{\mathrm{leaf}}$ 与 $\tau_{\mathrm{merge}}$。

自适应秩选择机制把“每个节点保留多少通道”转化成“每个节点允许损失多少相对能量”。
