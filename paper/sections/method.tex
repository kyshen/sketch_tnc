...
\subsection{小结}
综上，本文方法围绕一个统一对象展开：\emph{分隔状态}。
划分树规定了这些状态如何被组织，递归合并规定了它们如何在树上向上传播，低秩压缩则保证这一传播在计算上可控。
输出分块、根节点读出和块间缓存，都服务于这一主线。
因此，本文的方法可以概括为
\[
\text{划分树}
\;+
...
\text{递归合并与重压缩}.
\]
它近似的不是最终输出，而是在保持输出边界语义不变的前提下，对树上递归传播的内部接口状态做近似。
