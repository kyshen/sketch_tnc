...
\subsection{划分树与分隔状态}
为了统一组织一般图上的递归收缩，我们在张量网络的交互图上构造一棵二叉划分树（partition tree）。
...
\subsection{基于 sketch 的随机化压缩}
仅靠自适应秩选择还不够。
在内部节点处，若左右孩子的秩分别为 $r_L$ 和 $r_R$，则父节点在重压缩之前自然会出现一个秩乘积 $r_L r_R$。
如果先显式构造合并后的大接口矩阵，再对它做普通低秩分解，就会遇到“先膨胀，再压回去”的中间瓶颈。
为此，本文在合并节点上进一步采用基于 sketch 的随机化压缩：不显式收缩完整的接口矩阵，而是通过算子访问直接恢复其主导子空间\cite{halko2011finding,heddes2026approximating}。
...
直接采用固定秩可以直接保留前 $r$ 个奇异方向；而本文主线采用基于尾能量的自适应截断：给定合并容忍度 $\tau_{\mathrm{merge}}$，选择满足
\[
\left(
\frac{\sum_{i>r} \sigma_i^2}{\sum_i \sigma_i^2}
\right)^{1/2}
...
\subsection{输出分块、根节点读出与缓存}
除了上述核心递归外，完整收缩还需要三个与任务设定直接相关的实现机制。

\paragraph{输出分块。}
为了避免一次性面对完整输出空间，我们只对部分开放标签做分块。
对固定输出块 $\Omega_b$ 而言，若某些开放腿已被限制到较小的局部索引集，则各子树在输出侧的维度也会随之减小。
因此，输出分块并不是附加包装，而是直接影响分隔状态规模的组成部分。

\paragraph{根节点读出。}
当递归合并到根节点时，由于根节点对应整个网络，其边界标签为空。
这时根状态不再是一般的“输出侧 $\times$ 边界侧”接口矩阵，而是直接退化为当前输出块的稠密张量。
也就是说，分隔状态递归在根节点自然终止，并转化为当前输出块的读出。

\paragraph{块间缓存。}
在按块收缩时，不同输出块往往共享大量上游子树。
若每个块都从头构建整棵树上的所有状态，那么输出分块的潜在收益会被重复计算抵消。
因此，我们对分隔状态做块间缓存。
一个状态能否复用，不取决于全局块编号，而取决于该子树真正看到的局部块条件；只要两个输出块在某个子树上的局部切片完全一致，该子树状态就可以直接复用。
